...
public class BoardController extends HttpServlet {
	...
	ServletException, IOException {
		...
		if(url.indexOf("list.do") != -1){
		...
		}else if(url.indexOf("list2.do") != -1){
			//레코드 갯수 계산
			int count=dao.count();
			//페이지 나누기를 위한 처리
			int curPage=1;
			...
			//페이지 네비게이션 출력을 위한 정보 전달
			request.setAttribute("page", pager);
			String page="/board/list2.jsp";
			RequestDispatcher rd
			=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}else if(url.indexOf("head.do") != -1) {
			//레코드 갯수 계산
			int count=dao.count();
			//페이지 나누기를 위한 처리
			int curPage=1;
		...
		}else if(url.indexOf("Rambo.do") != -1) {
			int count=dao.count();
			//페이지 나누기를 위한 처리
			int curPage=1;
			//숫자 처리는 null포인트 익셉션이 잘 일어나기 때문에
